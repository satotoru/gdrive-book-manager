...
- **No SQL / No Local DB:** アプリケーションサーバーはステートレスに近い構成とし、永続化データはすべてGoogle Driveに依存する
- **Drive as KVS:** 書誌情報（メタデータ）はGoogle Driveのカスタムプロパティ（Properties）に保存し、検索もDrive API経由で行う
- **LAN Hosting:** 自宅サーバー（Raspberry PiやNAS上のDockerコンテナ等）で稼働させ、LAN内からのアクセスを主とする
