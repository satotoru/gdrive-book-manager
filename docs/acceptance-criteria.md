...
以下は要件定義書では未定義だった事項について、レビューにより**確定した仕様**の一覧である。各検収項目にはこれらの仕様が反映済みである。

| # | 仕様項目 | 確定内容 | 反映先検収項目 |
|---|---------|---------|---------------|
| Q1 | **重複ISBN登録時の挙動** | 警告を表示し、ユーザーの承認により上書きする | FLW-004 |
| Q2 | **複数著者の区切り文字** | ハイフン `-` で区切る（例: `著者A-著者B`） | DAT-005, DRV-007 |
| Q3 | **複数著者時のフォルダ構成** | 第一著者のフォルダに保存する（例: `/MyLibrary/著者A/`） | DRV-007 |
| Q4 | **プロパティ値の文字数制限対応** | 124バイト以内に切り詰めて保存する | DAT-008 |
| Q5 | **カバー画像の保存方法** | Google Driveに画像ファイルとしてアップロードして保存する | BRW-004, OPD-004, DRV-010, DRV-011, DAT-011 |
| Q6 | **書籍の削除機能** | Phase 1に含める。UIからの削除操作を提供する | DEL-001〜DEL-005 |
| Q7 | **書籍の編集機能** | Phase 1に含める。メタデータ編集機能を提供する | EDT-001〜EDT-006 |
| Q8 | **ページネーション仕様** | Google Drive APIの `nextPageToken` を利用した方式とする | BRW-008 |
| Q9 | **キャッシュの有効期限** | TTL 1時間 | PER-002 |
| Q10 | **アプリへのアクセス制御** | 認証なし。LAN内はオープンアクセスとする | AUT-005 |
| Q12 | **`概要` のプロパティ保存** | 実装者判断（カスタムプロパティまたはファイルのdescriptionフィールドのいずれか） | DAT-010 |
| Q13 | **`カバー画像URL` のプロパティ保存** | Google Drive上に画像ファイルとして別途保存する（外部URLへの依存を排除） | DAT-011, DRV-010, DRV-011 |
| Q14 | **対応ブラウザ** | Chrome最新版のみを動作保証対象とする | ENV-006 |

---

**本文書は要件定義書 v1.0 に基づいて作成されました。全未確定事項はレビューにより確定済みです（v2.0）。**
